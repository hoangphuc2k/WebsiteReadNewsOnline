...
@section('content')
<form action="{{route('Comment.update',$data[0]['IdCom'])}}" method="POST">
    @method('PATCH')
    @csrf
    <h3>SỬA BÌNH LUẬN</h3>
    <div class="form-group">
      <label for="formGroupExampleInput">Tiêu đề</label>
      <input type="text" value="{{$data[0]['Title']}}"  class="form-control @error('Title') is-invalid @enderror" value="{{ old('Title') }}" name="Title" required maxlength="1000" id="formGroupExampleInput" autocomplete="Title" placeholder="Tiêu đề...">  
      @error('Title')
